...
$repositorio_gestor = Repositorio.new
$cheques_gestor = GeneradorCheque.new(Date.today)

get '/' do
  @empleados_archivo = $repositorio_gestor.obtener_empleados_archivo
  @empleados_memoria = $repositorio_gestor.obtener_empleados_memoria
  erb :"index"
end
...
post '/crear_empleado' do 
  ...
  $repositorio_gestor.adicionar(empleado,$repositorio)
  @empleados_archivo = $repositorio_gestor.obtener_empleados_archivo
  @empleados_memoria = $repositorio_gestor.obtener_empleados_memoria
...
get '/edit/:ci' do
  @empleado = $repositorio_gestor.buscar_por_ci(params[:ci])
  erb :"edit"
end

get '/edit_archive/:ci' do
  @empleado = $repositorio_gestor.mostrar_de_archivo(params[:ci])
  erb :"edit_archive"
end

#edit
post '/actualizar_empleado/:ci' do
  empleado = $repositorio_gestor.buscar_por_ci(params[:empleado][:ci])
  ...
  if ($repositorio_gestor.actualizar(empleado_modificado, $repositorio))  
    @empleados_archivo = $repositorio_gestor.obtener_empleados_archivo
    @empleados_memoria = $repositorio_gestor.obtener_empleados_memoria
    erb :"index"
  end
...
#delete
get '/delete/:ci' do
  @empleado = $repositorio_gestor.eliminar(params[:ci],$repositorio)
  erb :"delete"
end
...
get '/add_check/:ci' do
  ...
  if @cheque != nil
    $repositorio_gestor.adicionar_cheque(@cheque)
    erb :"add_check"
  else
    erb :"not_check"  
  end
...
get '/checks' do
  ...
  @empleados_memoria.each do |empleado|
    cheque = $cheques_gestor.ejecutar(empleado)
    if $repositorio_gestor.adicionar_cheque(cheque)
      empleado.control_cheque(true)
      $repositorio_gestor.actualizar(empleado)
    end
  end
  @cheques = $repositorio_gestor.obtener_cheques
  erb :"checks"
end
...
post '/create_timecard/:ci' do
  @empleado = $repositorio_gestor.buscar_por_ci(params[:timecard][:id_empleado])
...
post '/create_servicecard/:ci' do
  @empleado = $repositorio_gestor.buscar_por_ci(params[:servicecard][:id_empleado])
